...
function Navigation() {
  ...
  const showPrijavaLink = windowWidth > 900 || menuOpen;
    ...
    <Navbar expand="lg" style={{ backgroundColor: 'hsla(232, 87%, 98%, 0.8)', height: '120px' }} variant="dark" className={`modern-navbar sticky-top ${menuOpen ? 'opened' : ''}`}>
      ...
      <Navbar.Collapse id="responsive-navbar-nav">
        <Nav className={`mr-auto ${menuOpen ? 'nav-open' : ''}`}>
         ...
         <Nav.Link href="/registrationfeesaccommodation/eventregistration" className={`nav-link ${menuOpen ? 'nav-link-open' : ''}`}>
           Prijava
         </Nav.Link>
       </Nav>
        )}
...
//       </Navbar.Collapse>
//       <Nav className={`ml-auto ${menuOpen ? 'nav-open' : ''}`}>
//   {/* Add the "hide-prijava" class to hide the link on smaller screens */}
//   <Nav.Link href="/registrationfeesaccommodation/eventregistration" className={`nav-link ${menuOpen ? 'nav-link-open hide-prijava' : ''}`}>
//     Prijava
//   </Nav.Link>
// </Nav>
